...
export default function HistoryCard({ item, onView }) {
  return (
    ...
    >
      ...
      <div className="p-4 sm:p-5 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/60 focus-visible:ring-offset-2 focus-visible:ring-offset-white rounded-b-2xl">
        {/* Header */}
        <div className="flex items-start justify-between gap-3 min-w-0">
          ...
          <span
            className={`flex-shrink-0 text-[11px] sm:text-xs px-2.5 py-1 rounded-full font-medium ${chipClasses[item.status] || chipClasses.Default
              }`}
          >
            {item.status}
          </span>
        ...
        <div className="mt-5 flex items-center justify-end">
          <span className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3.5 py-2 text-sm font-medium text-white 
                           hover:bg-emerald-700 motion-safe:transition-colors">
            View
            <svg className="w-4 h-4 group-hover:translate-x-0.5 motion-safe:transition-transform" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              <path
                fillRule="evenodd"
